...
//			}
		...
		writer.write("--" + BOUNDARY + "--" + CRLF);
		writer.close();
		return new buffer.readBinary().Resource(new $context.$rhino.mime.Type("multipart", subtype, { boundary: BOUNDARY }));
	} else {
		var subtype = p.subtype;
		var parts = p.parts;
		...
		return new $context.api.io.Resource({
			type: $context.$rhino.mime.Type.parse(String($multipart.getContentType())),
			read: {
				binary: function() {
					var buffer = new $context.api.io.Buffer();
...
$exports.Type = $context.$rhino.mime.Type;
...
$exports.Type.guess = function(p) {
	if (p.name) {
		...
		return $context.$rhino.mime.Type.parse(String(_rv));
	} else {
