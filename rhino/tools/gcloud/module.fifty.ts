...
namespace slime.jrunscript.tools.gcloud {
	export namespace cli {
		...
		export type Configuration = <P,R>(command: Command<P,R>) => {
			...
			handler: (events: slime.$api.Events<Events>) => slime.$api.event.Handlers<slime.jrunscript.shell.run.AskEvents>
			result: (result: slime.jrunscript.shell.run.Exit) => R
		}

		export type Executor = <P,R>(command: cli.Command<P,R>) => slime.$api.fp.world.Instrument<P,cli.Events,R>
	...
	export interface Exports {
		...
		cli: {
			Installation: {
				 ...
				 */
				create: slime.$api.fp.world.Operation<string,{
					console: string
				}>
			},
			Configuration: {
