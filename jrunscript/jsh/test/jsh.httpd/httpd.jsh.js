	...
	function(jsh) {
		...
		var helloServlet = new function() {
			this.test = function(url) {
				...
				if (!/^text\/plain/.test(response.body.type.toString())) {
		...
		var fileServlet = new function() {
			...
			this.test = function(url) {
				...
				jsh.shell.echo((response.body.type) ? response.body.type.toString() : "(null)");
				var code = {
		...
		var apiServlet = new function() {
			this.test = function(url) {
				...
				jsh.shell.echo(response.body.type.toString());
		...
		//			resources: jsh.httpd.Resources.script(jsh.script.getRelativePath("httpd.resources.js").file)
					resources: (function() {
						...
						jsh.loader.run(jsh.script.file.parent.getRelativePath("httpd.resources.js"), {
							$mapping: new function() {
								this.getRelativePath = function(path) {
									return jsh.script.file.parent.getRelativePath(path)
								}
							},
			...
			var run = function(path,tests) {
				...
				tomcat.map({
					...
					//	TODO	is there a jsh.script.getFile()?
					resources: jsh.httpd.Resources.script(jsh.script.file.parent.getRelativePath("httpd.resources.js").file)
				});
				tomcat.start();
