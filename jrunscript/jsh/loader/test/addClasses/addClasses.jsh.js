...
(
	/**
	 *
	 * @param { slime.jrunscript.Packages & { test: any } } Packages
	...
	function(Packages,$api,jsh) {
		...
		if (parameters.options.scenario) {
			...
			jsh.loader.plugins(jsh.script.file.parent.getRelativePath("../../../rhino/tools"));
			...
			jsh.java.tools.javac({
				...
				arguments: [
					jsh.script.file.parent.getRelativePath("java/test/AddClasses.java")
				]
			});
			...
			if (jsh.shell.jsh.home) {
				...
				jsh.shell.jsh({
					...
					arguments: (function(rv) {
						...
						var rhinoInstalled = library.local(library.directory(jsh.shell.jsh.lib.pathname.os.adapt()));
						if (!rhinoInstalled) {
							rv.push("-norhino");
						}
						return rv;
